pipeline {
    ...
    stages {
       ...
       stage ('Push to Docker Hub'){
           ...
           steps{
              script{
                   docker.withRegistry('https://hub.docker.com/repository/docker/carlosignaciom','docker_hub_login'){
      ...
      stage ('Deploy to pro'){
          ...
          steps{
              ...
              script{
                  sh "shpass -p $USERPASS" -v ssh -o StrictHostKeyChecking=no $USERNAME@$prod_ip \"docker pull carlosignaciom/trainschedule:${env.BUILD_NUMBER}\""
                  try {
